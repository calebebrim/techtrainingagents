...
const Sidebar: React.FC<{ isOpen: boolean }> = ({ isOpen }) => {
    const { user } = useAuth();
    const hasMgmtAccess = user?.roles.some(role => [UserRole.ADMIN, UserRole.COURSE_COORDINATOR].includes(role));
    const hasSystemAccess = user?.roles.includes(UserRole.SYSTEM_ADMIN);
